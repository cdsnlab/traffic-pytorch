...
class STGCNModel(nn.Module):
    def __init__(self, config, Lk):
        super(STGCNModel, self).__init__()
        
        self.st_conv1 = st_conv_block(config.Ks, config.Kt, config.num_nodes, config.blocks[0], config.droprate, Lk)
        self.st_conv2 = st_conv_block(config.Ks, config.Kt, config.num_nodes, config.blocks[1], config.droprate, Lk)
        self.output = output_layer(config.blocks[1][2], config.n_his - 4 * (config.Kt - 1), config.num_nodes)

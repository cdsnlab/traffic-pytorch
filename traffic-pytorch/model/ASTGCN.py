...
class ASTGCNModel(nn.Module):
    def __init__(self, config, cheb_polynomials):
        ...
        self.submodule = ASTGCN_submodule(config.device, config.nb_block, config.in_channels, config.K, config.nb_chev_filter,\
         config.nb_time_filter, config.time_strides, cheb_polynomials, config.num_for_predict, config.len_input, config.num_of_vertices)
